...
plotUnconstrained <- function(data, num_steps) {
  ...
  payoff_plot <- ggplot(data[data$adj_mat %in% unconstrained_matrices & data$step_payoff > 0,], aes(x = strategy, y = step_payoff, color = strategy)) +
  geom_jitter(position = position_jitter(width = 0.4, height = 0), alpha = 0.1) +
  stat_summary(fun = mean, geom = "crossbar", width = 0.5, fatten = 4, position = position_dodge(0.9)) +
  theme_minimal() +
  ...
  facet_wrap(~ step_factor, labeller = labeller(step_factor = function(value) paste0("Steps: Num Traits / ", value)))
...
all_data <- read_all(3:7)
all_data1$step_factor <- 1
all_data2$step_factor <- 2

all_data_combined <- rbind(all_data1, all_data2)
...
allsteps <- plotUnconstrained(all_data_combined[all_data_combined$num_nodes == 4, ], "")
