...
data <- readRDS("data_processed_allcombs_new.rds")
data <- data[!data$strategy == "Conformity",]
data <- rbind(data, confdata)

payoff_harsh <- plotDVbyIV_binned(
  data[data$alpha == 0 & data$strategy != "Similarity",],
  DV = "step_payoff", DV_label = "Performance",
  IV = "avg_path_length",  IV_label ="Constraints on Learning Order",
  lambda_value = 2,
...
plotDVbyIV_binned <- function(data, DV, DV_label, IV, IV_label, lambda_value, strategy_colors = NULL) {
  ...
  bin_centers <- seq(min_iv + 0.25, max_iv, by = 0.5)
  bin_edges <- data.frame(
    bin_start = bin_centers - 0.25,
    bin_end = bin_centers + 0.25,
    bin_center = bin_centers
  )

  bin_edges <- rbind(c(1,1.01,1),bin_edges)
  bin_edges[2,1] <- 1.01
  ...
  min_dv <- min(binned_data$binned_avg_DV, na.rm = TRUE)
  max_dv <- max(binned_data$binned_avg_DV, na.rm = TRUE)
  binned_data$binned_avg_DV <- (binned_data$binned_avg_DV - min_dv) / (max_dv - min_dv)
