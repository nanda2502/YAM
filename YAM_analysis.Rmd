...
plotDVByDistance <- function(data, DV, DV_label) {
  ...
  plot <- ggplot(data, aes(x = total_distance, y = data[, DV], color = as.factor(strategy))) +
    geom_point() +
    ...
    scale_y_continuous(
      breaks = seq(0, max(data[, DV]) + 1, by = 1),
      minor_breaks = seq(0, max(data[, DV]), by = 0.5)
      )
...
## plot 3x3 grid
outplot <- grid.arrange(grobs = plots_list, ncol = 3, top = textGrob("Expected Steps until Absorption for α = 1.0 "))
...
data <- read_file(5)
plot <- plot_graph_panel(data[is.nan(data$step_payoff) | is.infinite(data$steps), ], num_nodes)
```