...
read_file <- function(num_nodes) {
  ...
  colnames(data) <- c("num_nodes", "adj_mat", "alpha",
                      "strategy", "repl", "steps", "step_payoff", "step_transitions")
  return(data)
}
...
plotDVbyIV <- function(data, DV, DV_label, IV, IV_label, lambda_subset, alpha_subset) {
  average_data <- data %>%
    filter(lambda == lambda_subset,alpha == alpha_subset) %>%
  ...
  plot <- ggplot(average_data, aes_string(x = IV, y = "avg_DV", color = "as.factor(strategy)")) +
    ...
    labs(
      ...
      subtitle = paste0("Number of nodes: ", data$num_nodes[[1]],  ", Expected number of learning opportunities λ = ", lambda_subset, ", Depth bonus: ", alpha_subset),
      x = IV_label,
      y = DV_label
    ) +
    theme_minimal() +
...
saveRDS(data, "data_processed_allcombs.rds")
  ...
  for(num_nodes in node_range) {
    ...
    for (i in seq_len(nrow(data))) {
      if (count_processed %% 100 == 0) {
        print(paste("Processed", count_processed, "out of", nrow(data), "observations for", num_nodes, "nodes"))
      })
